...
const std::string CALIBRATION_PATH =
    (std::filesystem::path(__FILE__).parent_path() / ".." / ".." / ".." / "config"
        / "dex3_calibration.yaml")
        .lexically_normal().string();

constexpr const char* kRightThumbRotationKey = "right_thumb_rotation";
    ...
    : HandRetargeter(left_enabled, right_enabled, recording_label, raise_error)
{
    ...
    if (right_enabled) {
        ...
        // Resume the right thumb rotation from the last persisted value. A
        // missing/unreadable file just leaves the compiled-in default.
        try {
            const YAML::Node cal = YAML::LoadFile(CALIBRATION_PATH);
            if (cal[kRightThumbRotationKey])
                right_->set_thumb_rotation(cal[kRightThumbRotationKey].as<float>());
        } catch (const std::exception&) { /* no calibration yet — use default */ }
    }
...
void Dex3Retargeter::finish() {
    ...
    // Persist the operator-tuned right thumb rotation so the next run resumes
    // from it. Best-effort: a write failure must not derail shutdown.
    if (right_) {
        try {
            YAML::Node cal;
            cal[kRightThumbRotationKey] = right_->thumb_rotation();
            std::ofstream out(CALIBRATION_PATH, std::ios::trunc);
            out << "# Auto-generated by collect. Right Dex3 thumb_rotation (rad),\n"
                   "# tuned live with the arrow keys and saved on exit.\n"
                << cal;
        } catch (const std::exception&) { /* best-effort persistence */ }
    }
}
